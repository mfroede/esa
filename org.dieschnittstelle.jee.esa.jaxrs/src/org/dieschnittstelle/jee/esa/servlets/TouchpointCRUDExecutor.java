...
public class TouchpointCRUDExecutor {
	...
	public AbstractTouchpoint updateTouchpoint(final AbstractTouchpoint touchpoint) {
		int index = this.touchpoints.indexOf(new AbstractTouchpoint() {
			...
			@Override
			public int getId() {
				return touchpoint.getId();
			}

		});
		
		if (index == -1) {
			logger.warn("could not find touchpoint to be updated with id " + touchpoint.getId() + ". Will create a new one.");
			return createTouchpoint(touchpoint);
		}
				
		// we just replace the existing touchpoint with the new one
		this.touchpoints.remove(index);
		this.touchpoints.add(index,touchpoint);
