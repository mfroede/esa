 ...
 */
package org.dieschnittstelle.jee.esa.gae.server.gcm;

import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.GCM_SEND_ENDPOINT;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.JSON_CANONICAL_IDS;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.JSON_ERROR;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.JSON_FAILURE;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.JSON_MESSAGE_ID;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.JSON_MULTICAST_ID;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.JSON_PAYLOAD;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.JSON_REGISTRATION_IDS;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.JSON_RESULTS;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.JSON_SUCCESS;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.PARAM_COLLAPSE_KEY;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.PARAM_DELAY_WHILE_IDLE;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.PARAM_PAYLOAD_PREFIX;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.PARAM_REGISTRATION_ID;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.PARAM_TIME_TO_LIVE;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.TOKEN_CANONICAL_REG_ID;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.TOKEN_ERROR;
import static org.dieschnittstelle.jee.esa.gae.server.gcm.Constants.TOKEN_MESSAGE_ID;

import org.dieschnittstelle.jee.esa.gae.server.gcm.Result.Builder;
...
public class Sender {
			...
			try {
				...
				if (line == null || line.equals("")) {
					throw new IOException(
							"Received empty response from GCM service.");
				}
				...
				if (token.equals(TOKEN_MESSAGE_ID)) {
					...
					throw new IOException(
							"Received invalid response from GCM: " + line);
				}
			} finally {
				reader.close();
			}
	...
	public MulticastResult sendNoRetry(Message message,
		...
		HttpURLConnection conn = post(GCM_SEND_ENDPOINT, "application/json",
				requestBody);
		int status = conn.getResponseCode();
		String responseBody;
		if (status != 200) {
