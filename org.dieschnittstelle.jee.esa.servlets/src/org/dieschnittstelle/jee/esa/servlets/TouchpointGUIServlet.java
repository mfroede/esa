...
public class TouchpointGUIServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		if (pathInfo.startsWith("/delete")) {
			// if we have a delete request, the id to be deleted will be the last segment of the path
	...
	private void displayView(HttpServletRequest request,
			HttpServletResponse response) {
		/*
		 *  we read out the touchpoint data and add it as an attribute to the request such that the jsp can be agnostic with regard to how the data to be display has been obtained
		 */
		// obtain the executor
		TouchpointCRUDExecutor exec = (TouchpointCRUDExecutor)getServletContext().getAttribute("touchpointCRUD");
